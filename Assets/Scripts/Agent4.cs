...
public class Agent4 : Agent
{
    ...
    private IEnumerator ProcessRecipe()
    {
        ...
        // Si les ingrédients ne sont pas encore prêts, attendre
    ...
    private IEnumerator CookSoup(Recipe recipe)
    {
        ...
        {
            Debug.LogError("Marmite prefab introuvable pour Agent4.");
            yield break;
        }
    ...
    private IEnumerator CookMeat(Recipe recipe, Action<Ingredient> onCooked)
    {
        if (panPrefab == null)
        {
            Debug.LogError("Poêle prefab introuvable pour Agent4.");
            yield break;
        }
