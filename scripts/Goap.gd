...
class GoapPlanner:
	...
	func plan_for_recipe(recipe: Recipe) -> Dictionary:
		...
		for ing in recipe.ingredients:
			# Prendre l'ingrédient (implicite dans les sous-objectifs suivants)
			...
			match ing.state:
				...
				Ingredient.State.COOKED:
					# facultatif: coupe si nécessaire, puis cuisson
					subgoals.append("Amener %s à découpage" % ing.base_type)
...
func tick() -> void:
	...
	if current_action_step_index == -1:
		# mémoriser l'état de progression de recette au début de l'action
...
func _execute(action: GoapAction) -> bool:
	match action.kind:
		...
		"cook_plate":
			...
			if ps.is_cooking:
				return false
			if agent_owner.held_item and agent_owner.held_item.has_method("is_plate") and agent_owner.held_item.call("is_plate") and agent_owner.held_item.get("is_cooked"):
				return true
			if ps.plate != null and ps.plate.is_cooked and agent_owner.held_item == null:
				agent_owner.set_goap_target(ps.global_position)
				return agent_owner.held_item != null and agent_owner.held_item.has_method("is_plate") and agent_owner.held_item.call("is_plate") and agent_owner.held_item.get("is_cooked")
			return false
		"deliver":
...
func _pickup_from_source(ingredient_type: String) -> bool:
	...
	if target == null:
		return false
	agent_owner.set_goap_target(target.global_position)
...
func _use_station(key: String) -> bool:
	...
	if station == null:
		return true
	agent_owner.set_goap_target(station.global_position)
	match key:
		"cutting":
		...
		"plate":
			# succès quand la progression de recette a avancé (ingrédient déposé)
